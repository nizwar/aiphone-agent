...
protocol ADBProviding: Sendable {
    ...
    func listFiles(path: String, deviceID: String?) throws -> [ADBFileItem]
    func pullFile(remotePath: String, localPath: String, deviceID: String?) throws
    func pushFile(localPath: String, remotePath: String, deviceID: String?) throws
    func deleteFile(remotePath: String, recursive: Bool, deviceID: String?) throws
    func renameFile(oldPath: String, newPath: String, deviceID: String?) throws
}
...
final class ADBProvider: ADBProviding, @unchecked Sendable {
    ...
    func shell(_ command: String, deviceID: String? = nil) throws -> String {
        let result = try runADB(["shell", command], deviceID: deviceID)
    ...
    private func shellWithTimeout(_ command: String, deviceID: String? = nil, timeout: TimeInterval = 5) -> String? {
        let result = try? runADBWithTimeout(["shell", command], deviceID: deviceID, timeout: timeout)
    ...
    func listFiles(path: String, deviceID: String? = nil) throws -> [ADBFileItem] {
        let cleanPath = path.isEmpty ? "/" : path
        let result = try runADB(["shell", "ls -la \(shellQuote(cleanPath))"], deviceID: deviceID)
        let output = result.standardOutput.isEmpty ? result.standardError : result.standardOutput
        return parseLSOutput(output, basePath: cleanPath)
    }

    private func parseLSOutput(_ output: String, basePath: String) -> [ADBFileItem] {
        var items: [ADBFileItem] = []
        var seenPaths = Set<String>()

        for line in output.components(separatedBy: "\n") {
            let str = line.trimmingCharacters(in: .whitespaces)
            guard !str.isEmpty, !str.hasPrefix("total ") else { continue }

            let parts = str.split(separator: " ", maxSplits: 7, omittingEmptySubsequences: true)
            ...
            let date = String(parts[5])
            let time = String(parts[6])
    ...
    func pullFile(remotePath: String, localPath: String, deviceID: String? = nil) throws {
        let localURL = URL(fileURLWithPath: localPath)
        let parentDir = localURL.deletingLastPathComponent().path
        try FileManager.default.createDirectory(atPath: parentDir, withIntermediateDirectories: true)
        let result = try runADB(["pull", remotePath, localPath], deviceID: deviceID)
        if !result.succeeded {
            throw ADBProviderError.commandFailed(result.combinedOutput.isEmpty ? "Failed to pull file" : result.combinedOutput)
        }
    }

    func pushFile(localPath: String, remotePath: String, deviceID: String? = nil) throws {
        let result = try runADB(["push", localPath, remotePath], deviceID: deviceID)
        if !result.succeeded {
            throw ADBProviderError.commandFailed(result.combinedOutput.isEmpty ? "Failed to push file" : result.combinedOutput)
        }
    }

    func deleteFile(remotePath: String, recursive: Bool = false, deviceID: String? = nil) throws {
        let flags = recursive ? "-rf " : ""
        let result = try runADB(["shell", "rm \(flags)\(shellQuote(remotePath))"], deviceID: deviceID)
        if !result.succeeded {
            throw ADBProviderError.commandFailed(result.combinedOutput.isEmpty ? "Failed to delete" : result.combinedOutput)
        }
    }

    func renameFile(oldPath: String, newPath: String, deviceID: String? = nil) throws {
        let result = try runADB(["shell", "mv \(shellQuote(oldPath)) \(shellQuote(newPath))"], deviceID: deviceID)
        if !result.succeeded {
            throw ADBProviderError.commandFailed(result.combinedOutput.isEmpty ? "Failed to rename" : result.combinedOutput)
        }
    }
    ...
    func getDeviceDetails(deviceID: String? = nil) -> ADBDeviceDetails {
        ...
        let memInfo = shellWithTimeout("cat /proc/meminfo 2>/dev/null | head -n 3", deviceID: deviceID)
        let totalRAM = parseTotalRAM(from: memInfo)

        let dfOutput = shellWithTimeout("df /data 2>/dev/null | tail -n 1", deviceID: deviceID)
        let (availStorage, totalStorage) = parseStorageInfo(from: dfOutput)

        let uptimeOutput = shellWithTimeout("uptime 2>/dev/null", deviceID: deviceID)
        let uptime = parseUptime(from: uptimeOutput)

        let kernelOutput = shellWithTimeout("uname -r 2>/dev/null", deviceID: deviceID)
        let kernelVersion = kernelOutput?.trimmingCharacters(in: .whitespacesAndNewlines)

        let ipOutput = shellWithTimeout("ip route get 1 2>/dev/null | head -n 1", deviceID: deviceID)
        let ipAddress = parseIPAddress(from: ipOutput)

        let selinuxOutput = shellWithTimeout("getenforce 2>/dev/null", deviceID: deviceID)
        let seLinuxStatus = selinuxOutput?.trimmingCharacters(in: .whitespacesAndNewlines)

        let openGLRaw = properties["ro.opengles.version"]
        let openGLVersion = openGLRaw.flatMap { parseOpenGLVersion($0) }
        ...
        return ADBDeviceDetails(
            ...
            basebandVersion: properties["gsm.version.baseband"],
            totalRAM: totalRAM,
            availableStorage: availStorage,
            totalStorage: totalStorage,
            locale: properties["persist.sys.locale"] ?? properties["ro.product.locale"],
            timezone: properties["persist.sys.timezone"],
            seLinuxStatus: (seLinuxStatus?.isEmpty == false) ? seLinuxStatus : nil,
            encryptionState: properties["ro.crypto.state"],
            ipAddress: ipAddress,
            bluetoothName: properties["bluetooth.name"] ?? properties["ro.product.model"],
            uptime: uptime,
            openGLVersion: openGLVersion,
            kernelVersion: (kernelVersion?.isEmpty == false) ? kernelVersion : nil,
            usbConfig: properties["sys.usb.config"]
        )
    }
    ...
    /// Shell-quotes a path for use inside `adb shell` commands.
    ...
    private func runADBWithTimeout(_ arguments: [String], deviceID: String? = nil, timeout: TimeInterval = 5) throws -> ADBCommandResult {
        let process = Process()
        ...
        let waitQueue = DispatchQueue(label: "com.aiphone.adb.timeout.\(UUID().uuidString)")
        waitQueue.async {
        ...
        if semaphore.wait(timeout: deadline) == .timedOut {
            process.terminate()
            return ADBCommandResult(standardOutput: "", standardError: "timeout", exitCode: -1)
        }

        let stdout = String(decoding: stdoutReader(), as: UTF8.self)
        let stderr = String(decoding: stderrReader(), as: UTF8.self)
        return ADBCommandResult(standardOutput: stdout, standardError: stderr, exitCode: process.terminationStatus)
    }
    ...
    private func parseTotalRAM(from output: String?) -> String? {
        guard let output else { return nil }
        for rawLine in output.split(separator: "\n") {
            let line = rawLine.trimmingCharacters(in: .whitespacesAndNewlines)
            guard line.hasPrefix("MemTotal:") else { continue }
            let value = line.replacingOccurrences(of: "MemTotal:", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
            if let kb = Int(value.replacingOccurrences(of: " kB", with: "").trimmingCharacters(in: .whitespacesAndNewlines)) {
    ...
    private func parseStorageInfo(from output: String?) -> (String?, String?) {
        ...
        let totalKB = Int(components[1])
        let availKB = Int(components[3])
        let totalStr = totalKB.map { String(format: "%.1f GB", Double($0) / 1_048_576.0) }
        let availStr = availKB.map { String(format: "%.1f GB", Double($0) / 1_048_576.0) }
        return (availStr, totalStr)
    }

    private func parseUptime(from output: String?) -> String? {
        guard let output else { return nil }
        let trimmed = output.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }
        // "up" portion: e.g. "... up 2 days, 3:45, ..."
        if let upRange = trimmed.range(of: "up ") {
            let afterUp = trimmed[upRange.upperBound...]
            if let commaRange = afterUp.range(of: ", ", options: [], range: afterUp.startIndex..<afterUp.endIndex) {
                // Check if there's a second comma (load averages) and trim there
                let portion = afterUp[afterUp.startIndex..<commaRange.lowerBound]
                let rest = afterUp[commaRange.upperBound...]
                // If next part looks like time (contains ":" or "min") include it
                if rest.contains(":") || rest.contains("min") {
                    if let nextComma = rest.firstIndex(of: ",") {
                        return String(afterUp[afterUp.startIndex..<nextComma]).trimmingCharacters(in: .whitespacesAndNewlines)
                    }
                }
                return String(portion).trimmingCharacters(in: .whitespacesAndNewlines)
            }
            return String(afterUp.prefix(40)).trimmingCharacters(in: .whitespacesAndNewlines)
        }
        return nil
    }

    private func parseIPAddress(from output: String?) -> String? {
        guard let output else { return nil }
        let trimmed = output.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }
        // "... src 192.168.1.100 ..."
        if let srcRange = trimmed.range(of: "src ") {
            let afterSrc = trimmed[srcRange.upperBound...]
            let ip = afterSrc.prefix(while: { !$0.isWhitespace })
            return ip.isEmpty ? nil : String(ip)
        }
        return nil
    }

    private func parseOpenGLVersion(_ raw: String) -> String? {
        guard let intValue = Int(raw) else { return raw }
        let major = intValue >> 16
        let minor = intValue & 0xFFFF

...
enum OpenGLMModelOption: String, CaseIterable, Identifiable, Sendable {
    ...
    case autoglmPhone9BMultilingual = "autoglm-phone-9b-multilingual"
...
final class AISettingsStore: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        self.openGLMModel = defaults.string(forKey: AppSettingsKeys.openGLMModel) ?? OpenGLMModelOption.autoglmPhoneMultilingual.rawValue
        ...
        self.languageEnhancerModel = defaults.string(forKey: AppSettingsKeys.languageEnhancerModel) ?? ""
    ...
    func fetchOpenGLMModels() {
        ...
        Task {
            do {
                let models = try await AIEndpointValidator.fetchModels(
                    baseURL: openGLMServer,
                    apiKey: openGLMKey
                )
                ...
                if models.isEmpty {
                    ...
                    if !models.contains(openGLMModel) {
                        openGLMModel = models.first ?? openGLMModel
                    }
